...
if geom_path.suffix.lower() == ".gpkg":
    gdf_geom = gpd.read_file(geom_path)[["field_id", "geometry"]].copy()
else:
    gdf_geom = gpd.read_parquet(geom_path)[["field_id", "geometry"]].copy()
...
# Cover crop status
cover_col = f"cover_{YY}_2"
df["cover_crop_status"] = pd.to_numeric(df[cover_col], errors="coerce")
df["cover_crop_detected"] = np.where(df["cover_crop_status"] == 3, 1, 0)
...
df = (
    ...
    .merge(gdf_geom, on="field_id", how="left")
)

df["year"] = YEAR

# Convert to GeoDataFrame
gdf_out = gpd.GeoDataFrame(df, geometry="geometry", crs=gdf_geom.crs)
...
# SAVE OUTPUT
# =========================================================
out_path = OUT_DIR / "OH_covercrop_dashboard_2015.parquet"
